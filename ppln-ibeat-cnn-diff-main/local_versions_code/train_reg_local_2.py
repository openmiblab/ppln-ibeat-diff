...
with open(r"C:\Users\eic20eh\Downloads\ibeat_local_test\dataset_monai.json", "r") as f:
    config = json.load(f)

output_root = r"C:\Users\eic20eh\Downloads\ibeat_local_test\testing_outputs"
...
LoadImaged(keys=["moving", "fixed"]),
    ...
    Resized(keys=["moving", "fixed"], spatial_size=(128, 128, 64), mode="bilinear"),
...
for epoch in range(num_epochs):
    ...
    for batch_data in train_loader:
        ...
        ddf = model(torch.cat((fixed, moving), dim=1))
        # Warp the moving image using the predicted field
        warped_image = warp_layer(moving, ddf)
        
        # Calculate Loss
        loss = image_loss(warped_image, fixed) + 0.01 * regularization_loss(ddf)
        loss.backward()
        optimizer.step()
    ...
    if (epoch + 1) % 1 == 0:
        ...
        with torch.no_grad():
            for i, val_data in enumerate(val_loader):
                ...
                if (epoch +1) == num_epochs and i < 3:  # Save outputs for the last epoch only
                    ...
                    for k in ["moving", "fixed", "warped"]:
                        ...
                        item[k].meta["filename_or_obj"] = os.path.join("val_results", f"val_case_{i}_{k}.nii.gz")
...
with torch.no_grad():
    for i, test_data in enumerate(test_loader):
        ...
        t_ddf = model(torch.cat((t_fixed, t_moving), dim=1))
        ...
        ref_path = item["fixed"].meta["filename_or_obj"]
        subject_name = os.path.basename(os.path.dirname(ref_path))

        for k in ["moving", "fixed", "warped"]:
            if k == "warped":
                item["warped"].copy_meta_from(item["fixed"])
            
            item[k].meta["filename_or_obj"] = os.path.join("test_results", f"{subject_name}_{k}.nii.gz")

        test_saver(item)
        print(f"Saved results for test case {i}")