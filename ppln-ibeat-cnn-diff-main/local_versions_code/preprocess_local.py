...
def run_pipeline(sourcedir, outputdir):
    ...
    for i, pair in enumerate(passed_pairs):
        ...
        moving_files = glob.glob(os.path.join(src_path, "**/dwi_unregistered.nii.gz"), recursive=True)
        fixed_files = glob.glob(os.path.join(src_path, "**/dwi_registered.nii.gz"), recursive=True)
        
        if not moving_files or not fixed_files:
            print(f"Skipping {pair['id']}: NIfTI files not found in source.")
            continue
            
        new_moving = moving_files[0].replace("\\", "/")
        new_fixed = fixed_files[0].replace("\\", "/")
...
def make_path_safe(path):
    # Converts to an absolute path and adds the Windows long-path prefix
...
if __name__ == "__main__":
    SOURCE = make_path_safe(r"C:\Users\eic20eh\Downloads\ibeat_local_test\Cleaned_data")
    OUTPUT = make_path_safe(r"C:\Users\eic20eh\Downloads\ibeat_local_test")
